...
export default function Literary() {
  ...
  useEffect(() => {
    const fetchLiteraryArticles = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('/api/categories/literary/articles');
        setArticles(response.data.data || []);
        
        // Fetch related articles for "More from this Category" section
        const relatedResponse = await axios.get('/api/categories/literary/articles');
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="bg-news-bg bg-cover bg-center h-20" style={{
        backgroundImage: `linear-gradient(to right, #228B22 20%, rgba(34,139,34,0.2)), url('/bg.jpg' )`
      }}>
