...
export default function News() {
  ...
  useEffect(() => {
    const fetchNewsArticles = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('/api/categories/news/articles');
        setArticles(response.data.data || []);
        
        // Fetch related articles for "More from this Category" section
        const relatedResponse = await axios.get('/api/categories/news/articles');
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="bg-news-bg bg-cover bg-center h-20" style={{
          backgroundImage: `linear-gradient(to right, #2a5a82 20%, rgba(42,90,130,0.2)), url('/bg.jpg' )`
        }}>
